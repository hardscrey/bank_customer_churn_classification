...
Переход к [**проекту .ipynb**](bank_customer_churn_prediction.ipynb)
## Содержание
- EDA и визуализация
...
## 📘 Описание проекта
Цель проекта — провести анализ клиентской базы банка и построить модель, предсказывающую, покинет ли клиент банк.  
Проект направлен на выявление факторов, влияющих на отток, и создание объяснимой модели, которую можно использовать для повышения удержания клиентов.

Все артефакты сохраняются в [`images/`](images/), [`results/`](results/) и [`model_comparison/`](model_comparison/).
...
> Она сочетает высокую точность и устойчивость, показывая сбалансированные результаты между train/test.

---

## 📊 Этап 7. ROC и Precision-Recall кривые

Для визуальной оценки разделения классов были построены:
...
## 🧮 Этап 8. Confusion Matrix и Classification Report

Построена **матрица ошибок** для лучшей модели (XGBoost):

📊 *[Вставь confusion matrix]*

| Метрика | Precision | Recall | F1-score |
|----------|-----------:|--------:|----------:|
| Ушедшие (1) | 0.81 | 0.77 | 0.79 |
| Оставшиеся (0) | 0.85 | 0.88 | 0.86 |
...
## 🧠 Этап 9. Интерпретация модели (SHAP-анализ)
...
📈 *[Вставь SHAP summary plot и bar plot feature importance]*

**Ключевые факторы:**
- `Age` — клиенты старше 40 лет чаще покидают банк.  
- `NumOfProducts` — большее количество продуктов (>2) увеличивает риск ухода.  
- `IsActiveMember` — активность клиента снижает вероятность оттока.  
- `BalanceGeoDiff` — значительное отклонение баланса от среднего по стране связано с нестабильным поведением клиента.

---

## 📊 Этап 10. Финальные результаты и инсайты

| Фактор | Влияние на отток | Бизнес-интерпретация |
|---------|------------------|----------------------|
| **Возраст** | Увеличивает риск | Клиенты старше 40 лет — целевая группа для удержания |
| **Количество продуктов** | Нелинейное влияние | Клиенты с 3–4 продуктами чаще уходят |
| **Активность** | Снижает риск | Важно стимулировать активность (мобильное приложение, акции) |
| **Баланс** | Слишком высокий баланс повышает риск | Анализировать “богатых, но неактивных” клиентов |

---

## 💬 Итоговые выводы
- Модель XGBoost обеспечивает наилучший результат (AUC ≈ 0.87).  
...
## 📘 Описание проекта
Проект посвящён анализу поведения клиентов банка и прогнозированию их оттока с помощью моделей машинного обучения.  
Цель — определить, какие факторы влияют на решение клиента покинуть банк, и построить интерпретируемую модель, способную предсказать вероятность ухода.

Данные: **Churn_Modelling.csv**  
Объём: 10 000 клиентов (демографические и финансовые признаки).  
...
- Удалены неинформативные поля: `RowNumber`, `CustomerId`, `Surname`.  
- Закодированы категориальные признаки: `Geography`, `Gender`.  
- Добавлен новый признак **BalanceGeoDiff** — отклонение баланса клиента от среднего по стране.  
- Проверены пропуски, выбросы и распределения.

> 💡 *Почему важно:*  
> Новая метрика BalanceGeoDiff помогает выявить клиентов с аномально высоким или низким балансом относительно региональной нормы, что может быть связано с риском оттока.
...
#### Анализ признаков
- **Возраст**: риск оттока растёт после 40 лет.  
- **Количество продуктов**: клиенты с 3–4 продуктами чаще уходят.  
- **Активность**: активные клиенты (IsActiveMember=1) уходят реже.  
- **Баланс**: клиенты с аномально высоким балансом по сравнению с регионом чаще уходят.
...
Целевая переменная `Exited` несбалансирована (≈20% уходящих клиентов).  
Для обучения моделей использован **Downsampling** мажорного класса, чтобы улучшить устойчивость классификации.
...
| Модель | Train AUC | Test AUC | Особенности |
|--------|-----------:|----------:|--------------|
| Logistic Regression | 0.84 | 0.83 | Базовая линейная модель |
| Random Forest | 0.91 | 0.86 | Нелинейная, объяснимая |
| XGBoost | **0.92** | **0.87** | Лучшая модель по AUC |
...
### 5. Интерпретация моделей (SHAP-анализ)
Использован **SHAP** для объяснимости XGBoost-модели.  
SHAP-значения показывают, как каждый признак влияет на вероятность оттока.

![](images/shap_summary.png)

**Ключевые выводы SHAP:**
- `Age` — старшие клиенты имеют повышенный риск ухода.  
- `NumOfProducts` — более 2 продуктов → риск растёт.  
- `IsActiveMember` — активность снижает вероятность ухода.  
- `BalanceGeoDiff` — аномально высокий баланс усиливает риск.  
...
> Модель улавливает около 77% уходящих клиентов, сохраняя при этом высокую точность классификации.

---

### 7. Финальные инсайты
| Фактор | Влияние на отток | Рекомендация |
|---------|------------------|--------------|
| **Возраст** | Увеличивает риск | Таргетировать клиентов 40+ удерживающими предложениями |
| **Количество продуктов** | Нелинейное влияние | Пересмотреть продуктовую политику для клиентов с 3–4 продуктами |
| **Активность клиента** | Снижает риск | Усиливать вовлечённость через активности и программы лояльности |
| **Баланс** | Высокий баланс → больший риск | Анализировать финансовое поведение “богатых, но пассивных” клиентов |

---

## 📈 Основные результаты

| Модель | Accuracy | AUC | F1 | Precision | Recall |
|---------|-----------:|----:|-----------:|--------:|
...
## 💬 Выводы
- Отток клиентов связан с возрастом, количеством продуктов и уровнем активности.  
- После балансировки классов и подбора гиперпараметров **XGBoost** показывает наилучшее качество и стабильность.  
- SHAP-анализ подтвердил ключевые закономерности и дал интерпретации на уровне отдельных клиентов.  
- Для бизнеса — приоритетными направлениями удержания являются **возрастная аудитория 40+** и **увеличение клиентской активности**.
